...
def random_rotate(degrees, axis):
    degree = math.pi * random.uniform(*degrees) / 180.0
    sin, cos = math.sin(degree), math.cos(degree)
    #assert pts[0].shape[1] == 3
    if axis == 0:
    ...
    return matrix
    #return torch.matmul(matrix, pts[0].unsqueeze(-1)).squeeze(-1), torch.matmul(matrix, pts[1].unsqueeze(-1)).squeeze(-1)


class SO3Rotate(object):
    """
    arbitrary rotation
    """
    def __call__(self, pc, norm):
        bsize = pc.size()[0]
        rotation_matrix2 = random_rotate((-180, 180), 2)
        rotation_matrix1 = random_rotate((-180, 180), 1)
        rotation_matrix0 = random_rotate((-180, 180), 0)
        for i in range(bsize):
            cur_pc = pc[i, :, :]
            cur_pc = torch.matmul(rotation_matrix2, cur_pc.unsqueeze(-1))
            cur_pc = torch.matmul(rotation_matrix1, cur_pc)
            cur_pc = torch.matmul(rotation_matrix0, cur_pc)
            cur_pc = cur_pc.squeeze(-1)
            pc[i, :, :] = cur_pc
        for i in range(bsize):
            cur_pc = norm[i, :, :]
            cur_pc = torch.matmul(rotation_matrix2, cur_pc.unsqueeze(-1))
...
class ZRotate(object):
    """
    rotation about the z axis
    """
    def __call__(self, pc, norm):
        bsize = pc.size()[0]
        rotation_matrix = random_rotate((-180, 180), 2)
        for i in range(bsize):
            cur_pc = pc[i, :, :]
            cur_pc = torch.matmul(rotation_matrix, cur_pc.unsqueeze(-1)).squeeze(-1)
            pc[i, :, :] = cur_pc
        for i in range(bsize):
            cur_pc = norm[i, :, :]
            cur_pc = torch.matmul(rotation_matrix, cur_pc.unsqueeze(-1)).squeeze(-1)
