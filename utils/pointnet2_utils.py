...
class QueryAndGroup(nn.Module):
    ...
    def forward(
        ...
        normal_trans = normal.transpose(1,2).contiguous()
        grouped_xyz = grouping_operation(
            xyz_trans, idx
        )  # (B, 3, npoint, nsample)
        grouped_normal = grouping_operation(
            normal_trans, idx
        )  # (B, 3, npoint, nsample)
        ...
        if features is not None:
            ...
            if self.use_xyz:
                new_features = torch.cat([raw_grouped_xyz, grouped_normal, grouped_xyz, grouped_features],
                                         dim=1)  # (B, C + 3 + 3, npoint, nsample)
            else:
                new_features = torch.cat([raw_grouped_xyz, grouped_normal, grouped_xyz, grouped_features],
                                         dim=1)#grouped_features
        else:
            assert self.use_xyz, "Cannot have not features and not use xyz as a feature!"
            new_features = torch.cat([raw_grouped_xyz, grouped_normal, grouped_xyz], dim = 1)
